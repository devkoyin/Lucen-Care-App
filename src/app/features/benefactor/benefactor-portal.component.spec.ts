...
describe('BenefactorPortalComponent', () => {
  ...
  it('returns to the benefactor sign-in page on sign out', () => {
    setup('active');
    const navigate = spyOn(TestBed.inject(Router), 'navigate');

    component.handleSignOut();

    expect(navigate).toHaveBeenCalledWith(['/auth', 'benefactor', 'login']);
  });

  it('links to settings once the account is verified', () => {
    setup('active');
    expect(component.settingsRoute()).toBe('/benefactor/settings');
  });

  // The settings route is verifiedGuard-ed, so offering the link while pending
  // would bounce the user straight back to /benefactor/pending.
