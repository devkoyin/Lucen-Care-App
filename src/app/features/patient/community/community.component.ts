...
@Component({
  ...
  standalone: true,
  imports: [NewPostModalComponent, CreateCommunityModalComponent],
...
export class CommunityComponent {
  ...
  readonly joinedCount = computed(() => this.groups().filter(g => g.joined).length);

  readonly stats = computed(() => [
    { value: '5.4K+',                          label: 'Members',           icon: '🤝' },
    { value: '142',                            label: 'Posts this week',   icon: '📝' },
    { value: '28',                             label: 'Active discussions', icon: '💬' },
    { value: String(this.joinedCount()),       label: 'Your groups',       icon: '⭐' },
  ]);
